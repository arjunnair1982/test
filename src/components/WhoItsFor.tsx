export function WhoItsFor() {
  const audiences = [
    ...
    },
    {
      icon: (
        <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h.01M11 7h2m-2 4h.01M5 7h1m-1 4h.01M9 11h1m-1 4h.01M5 11h1m-1 4h.01" />
        </svg>
      ),
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-8">
          {audiences.map((audience) => (
            <div
              key={audience.title}
              className="text-center p-8 rounded-xl bg-gray-50 border border-gray-100"
            >
              <div className="w-14 h-14 rounded-xl bg-dark/5 flex items-center justify-center text-dark mx-auto mb-5">
                {audience.icon}
              </div>
              <h3 className="text-lg font-semibold text-dark">{audience.title}</h3>
              <p className="mt-2 text-sm text-gray-500 leading-relaxed">{audience.description}</p>
            </div>
          ))}
        </div>
      </div>
